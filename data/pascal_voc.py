#   Taken from https://github.com/martius-lab/blackbox-deep-graph-matching
import pickle
import random
    ...
    def get_k_samples(self, idx, k, mode, cls=None, shuffle=True, num_iterations=200):
        ...
        if mode == "superset" and k == 2:  # superset sampling only valid for pairs
            ...
            for i in range(num_iterations):
                xml_used = list(random.sample(self.xml_list[cls], 3))
                anno_dict_1, anno_dict_2, anno_dict_3 = [self.__get_anno_dict(xml, cls) for xml in xml_used]
                ...
                kp_names_3 = [keypoint["name"] for keypoint in anno_dict_3["keypoints"]]
                kp_names_filtered = set(kp_names_1).intersection(kp_names_2).intersection(kp_names_3)
                anno_dict_1["keypoints"] = [kp for kp in anno_dict_1["keypoints"] if kp["name"] in kp_names_filtered]
                anno_dict_2["keypoints"] = [kp for kp in anno_dict_2["keypoints"] if kp["name"] in kp_names_filtered]
                anno_dict_3["keypoints"] = [kp for kp in anno_dict_3["keypoints"] if kp["name"] in kp_names_filtered]
                
                anno_list = [anno_dict_1, anno_dict_2, anno_dict_3]
        ...
        perm_mat_list = [
            np.zeros([len(_["keypoints"]) for _ in anno_pair], dtype=float) for anno_pair in lexico_iter(anno_list)
        ]
