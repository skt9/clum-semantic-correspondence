...
def stitch_images_side_by_side(img1, img2):
...
def visualize_keypoints(img_list: List[np.ndarray], kpts: List[np.ndarray], 
             n_pts: List[np.ndarray], perm_mat: np.ndarray, markersize: int=20):
    """
        Visualize keypoints for the two sets of points
    ...
    im0, im1 = img_list[0], img_list[1]
    kpt0, kpt1 = kpts[0], kpts[1]
    npts0, npts1 = n_pts[0], n_pts[1]

    colors = cm.rainbow(np.linspace(0, 1, np.max([npts0,npts1]) ))
    # stitched_image = stitch_images_side_by_side(img_list)
    inds_left, inds_right = np.where(perm_mat>0)    
    kpt0, kpt1 = kpt0[inds_left,:], kpt1[inds_right,:]
    ...
    ax[0].imshow(im0)
    ax[0].axis('off')
    for kpt, clr in zip(kpt0,colors):
        ax[0].scatter(kpt[0], kpt[1],marker='.',s=(markersize)**2,color=clr)
    ax[1].imshow(im1)
    ax[1].axis('off')
    for kpt, clr in zip(kpt1,colors):
        ax[1].scatter(kpt[0], kpt[1],marker='.',s=(markersize)**2,color=clr)
