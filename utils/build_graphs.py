...
def build_graphs(P_np: np.ndarray, n: int, n_pad: int = None, edge_pad: int = None):
    ...
    A = delaunay_triangulate(P_np[0:n, :])  #   Computes adjacency matrix
    edge_num = int(np.sum(A, axis=(0, 1)))  #   Compute the dge number
    ...
    edge_list = [[], []]
    features = []
    #   Go through the n nodes
    for i in range(n):                  #   Go through nodes
        for j in range(n):              #   
            if A[i, j] == 1:            #   Find value edges
                edge_list[0].append(i)  #   Insert into the edge list node i
                edge_list[1].append(j)  #   Insert into the edge list node j
